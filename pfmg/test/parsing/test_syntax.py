...
import pfmg.parsing.utils as syn
...
@pytest.mark.parametrize("term", [
    ...
    "PP{",
    "PP}",
    "PP{}",  # Dans une regex classique, c'est valide, mais ne mène à rien.
    "PP/{}",
    "PP/{,}",
...
def test_syntax(term, expected) -> None:
...
@parametrize
def test_parse_features(accords, accumulator, expected) -> None:
...
@parametrize
def test_parse_features_erreurs(accords, accumulator, error) -> None:
    with pytest.raises(error):
        syn.parse_features(accords, accumulator)


parametrize = pytest.mark.parametrize(
    "percolation, accords, accumulator, expected", [
        ("",
         [FeatStruct(Source=FeatStruct(SGenre="m")),
          FeatStruct(Source=FeatStruct()),
          FeatStruct(Source=FeatStruct(SCas="Nom"))],
        ...
        (";",
         [FeatStruct(Destination=FeatStruct(SGenre="m")),
          FeatStruct(Destination=FeatStruct(SCas="Nom"))],
         FeatStruct(Destination=FeatStruct()),
         FeatStruct(Destination=FeatStruct())),
...
parametrize = pytest.mark.parametrize(
    "percolation, accords, accumulator, error", [
...
@parametrize
def test_parse_percolation_erreurs(
        percolation,
        accords,
        accumulator,
        error
...
parametrize = pytest.mark.parametrize(
    "syntagme, traduction, f_accords, f_percolation, expected", [
...
@parametrize
def test_parse_one_rule(lhs, syntagme, accords, percolation, traduction,
                        expected) -> None:
...
def test_parse_config() -> None:
    yaml_filename = (Path(__file__).parent
                     / "data_for_test"
                     / "MorphoSyntax.yaml")
    with open(yaml_filename) as fh:
        config = yaml.safe_load(fh)
    start, source_prods, dest_prods = syn.parse_config(config)
    assert False
