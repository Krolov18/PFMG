...
def test_equal(fx_df_phonology, rule, expected, sigma) -> None:
    gabarit = Gabarit(rule=rule, sigma=sigma, phonology=fx_df_phonology)

    # test d'identité
    assert gabarit == gabarit

    # test d'égalité
    other_gabarit = Gabarit(rule=rule, sigma=sigma, phonology=fx_df_phonology)
    assert gabarit == other_gabarit

    # test avec le sigma qui diffère
    other_gabarit = Gabarit(
        rule=rule, sigma=frozendict(Genre="f"), phonology=fx_df_phonology
    )
    assert gabarit != other_gabarit

    # test avec le rule qui diffère
    other_gabarit = Gabarit(rule="5V44aV66", sigma=sigma, phonology=fx_df_phonology)
    assert gabarit != other_gabarit

    # test avec rule et sigma qui diffèrent
    other_gabarit = Gabarit(
        rule="5V44aV66", sigma=frozendict(), phonology=fx_df_phonology
    )
    assert gabarit != other_gabarit
...
def test_suffix_typeerror(fx_df_phonology) -> None:
    ...
    with pytest.raises(TypeError):
        _ = Gabarit(rule="", sigma=frozendict(Genre="m"), phonology=fx_df_phonology)

    with pytest.raises(TypeError):
        _ = Gabarit(
            rule="hHPJLK", sigma=frozendict(Genre="m"), phonology=fx_df_phonology
        )
...
def test_to_string(fx_df_phonology) -> None:
    # rule avec 'V'
    gabarit = Gabarit(
        rule="5V44aV66", sigma=frozendict(Genre="m"), phonology=fx_df_phonology
    )
    ...
    # rule avec 'U'
    gabarit = Gabarit(
        rule="5U44aV66", sigma=frozendict(Genre="m"), phonology=fx_df_phonology
    )
    ...
    # rule avec 'A'
    gabarit = Gabarit(
        rule="5A44aV66", sigma=frozendict(Genre="m"), phonology=fx_df_phonology
    )
    ...
    # rule avec 789
    gabarit = Gabarit(
        rule="5A44aV68", sigma=frozendict(Genre="m"), phonology=fx_df_phonology
    )
