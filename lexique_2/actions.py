...
def check_if_datapath_exists(namespace: argparse.Namespace) -> None:
    path = Path(namespace.datapath)
    if not (path.exists() and path.is_dir()):
        raise argparse.ArgumentTypeError(f"{namespace.datapath} is not a valid path")


def check_if_datapath_contains_all_files(namespace: argparse.Namespace) -> None:
    """
    ...
    if not (namespace.datapath / "Gloses.yaml").exists():
        raise argparse.ArgumentTypeError(f"{namespace.datapath} does not contain Gloses.yaml")
    if not (namespace.datapath / "Blocks.yaml").exists():
        raise argparse.ArgumentTypeError(f"{namespace.datapath} does not contain Blocks.yaml")
    if not (namespace.datapath / "Stems.yaml").exists():
        raise argparse.ArgumentTypeError(f"{namespace.datapath} does not contain Stems.yaml")
    if not (namespace.datapath / "Phonology.yaml").exists():
        raise argparse.ArgumentTypeError(f"{namespace.datapath} does not contain Phonology.yaml")


def check_yaml_files_with_cue(namespace: argparse.Namespace) -> None:
    """
    Comme "schemas" est un package cue, il faut placer le cwd dans le package,
    ...
    vet.files(os.path.join("schemas", "gloses.cue"), namespace.datapath / "Gloses.yaml")
    vet.files(os.path.join("schemas", "blocks.cue"), namespace.datapath / "Blocks.yaml")
    vet.files(os.path.join("schemas", "stems.cue"), namespace.datapath / "Stems.yaml")
    # vet.files(os.path.join("schemas", "phonology.cue"), namespace.datapath / "Phonology.yaml")
    os.chdir(path)


def lexicon_action(namespace: argparse.Namespace) -> None:
    path = Path(namespace.datapath)
    # vérifier que l'archive namespace.datapath existe et que seuls les fichiers suivants sont présents
    # - Gloses.yaml
    # - Blocks.yaml
    # - Stems.yaml
    # - Phonology.yaml
