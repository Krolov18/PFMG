...
from lexique_2.lexical_structures.Circumfix import Circumfix
...
def test_circumfix_error(rule, expected, sigma) -> None:
    ...
    with pytest.raises(TypeError):
        _ = Circumfix(rule=rule, sigma=sigma, phonology=phonology)
...
def test_get_rule() -> None:
    phonology = Phonology(apophonies=frozendict(Ø="i", i="a", a="u", u="u", e="o", o="o"),
                          ...
                          consonnes=frozenset("ptkbdgmnNfsSvzZrljw"),
                          voyelles=frozenset("iueoa"))
    circumfix = Circumfix(rule="a+X+d", sigma=frozendict(Genre="m"), phonology=phonology)
...
def test_equal() -> None:
    ...
    circumfix = Circumfix(rule="a+X+d", sigma=frozendict(Genre="m"), phonology=phonology)
    other_circumfix = Circumfix(rule="a+X+d", sigma=frozendict(Genre="m"), phonology=phonology)
    assert circumfix == other_circumfix

    other_circumfix = Circumfix(rule="a+X+d", sigma=frozendict(Genre="f"), phonology=phonology)
    assert circumfix != other_circumfix

    other_circumfix = Circumfix(rule="a+X+t", sigma=frozendict(Genre="m"), phonology=phonology)
    assert circumfix != other_circumfix
