...
from lexique_2.lexical_structures.Condition import Condition
...
def test_condition(rule, expected, sigma) -> None:
    ...
    condition = Condition(rule=rule, sigma=sigma, phonology=phonology)
    ...
    # test d'égalité
    other_condition = Condition(rule=rule, sigma=sigma, phonology=phonology)
    assert condition == other_condition

    # test avec le sigma qui diffère
    other_condition = Condition(rule=rule, sigma=frozendict(Genre="f"), phonology=phonology)
    assert condition != other_condition

    # test avec le rule qui diffère
    other_condition = Condition(rule="X3?X2:X1", sigma=sigma, phonology=phonology)
    assert condition != other_condition

    # test avec rule et sigma qui diffèrent
    other_condition = Condition(rule="X3?X2:X1", sigma=frozendict(), phonology=phonology)
    assert condition != other_condition
...
def test_to_string_with_none() -> None:
    phonology = Phonology(apophonies=frozendict(Ø="i", i="a", a="u", u="u", e="o", o="o"),
                          ...
                          consonnes=frozenset("ptkbdgmnNfsSvzZrljw"),
                          voyelles=frozenset("iueoa"))
    condition = Condition(rule="X2?X1:X1", sigma=frozendict(Genre="m"), phonology=phonology)
