...
from lexique_2.lexical_structures.Prefix import Prefix
...
def test_prefix(rule, expected, sigma) -> None:
    ...
    prefix = Prefix(rule=rule, sigma=sigma, phonology=phonology)
    ...
    # test d'égalité
    other_prefix = Prefix(rule=rule, sigma=sigma, phonology=phonology)
    assert prefix == other_prefix

    # test avec le sigma qui diffère
    other_prefix = Prefix(rule=rule, sigma=frozendict(Genre="f"), phonology=phonology)
    assert prefix != other_prefix

    # test avec le rule qui diffère
    other_prefix = Prefix(rule="a+X", sigma=sigma, phonology=phonology)
    assert prefix != other_prefix

    # test avec rule et sigma qui diffèrent
    other_prefix = Prefix(rule="a+X", sigma=frozendict(), phonology=phonology)
    assert prefix != other_prefix
...
def test_prefix_type_error() -> None:
    ...
    with pytest.raises(TypeError):
        Prefix(rule="", sigma=frozendict(), phonology=phonology)

    with pytest.raises(TypeError):
        Prefix(rule="hHPJLK", sigma=frozendict(), phonology=phonology)
...
def test_get_sigma() -> None:
    phonology = Phonology(apophonies=frozendict(Ø="i", i="a", a="u", u="u", e="o", o="o"),
                          ...
                          consonnes=frozenset("ptkbdgmnNfsSvzZrljw"),
                          voyelles=frozenset("iueoa"))
    prefix = Prefix(rule="d+X", sigma=frozendict(Genre="m"), phonology=phonology)
